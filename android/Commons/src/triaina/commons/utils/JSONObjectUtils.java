...
public final class JSONObjectUtils {
    ...
    /**
     * Compares 2 JSON objects. It compares the actual JSON data, and considers
     * a NULL or non-specified value for a key equivalent to an absence of key
     * (i.e. "{key:null}" is considered equivalent to "{}"). Not that values are
     * not coerced for comparison, so "{key:10}" isn't equivalent to
     * "{key:'10'}".
     * 
     * @param left
     *            The first JSON data to compare
     * @param right
     *            The JSON data to compare to {@link left}
     * @return true if the 2 JSON data are equivalent per the above description,
     *         false otherwise
     */
    public static boolean areDataEquivalent(final JSONObject left, final JSONObject right) {
        ...
        final String[] keysToCompare = new String[left.length() + right.length()]; // will
                                                                                   // contain
                                                                                   // all
                                                                                   // keys
                                                                                   // to
        ...
        for (i = 0; i < keysToCompare.length; ++i) {
            ...
            try {
                ...
                throw new JSONRuntimeException(jE); // shouldn't happen as we
                                                    // check for has() before
                                                    // calling get()
            }
            Object rightObj;
            try {
            ...
            // Android's JSONArray overrides equals() so no need for special
            // case for it
        }
