...
/**
 * - Note The field convert name convention example is following. mSomeField <->
 * some_field someField <-> some_field some_field <-> some_field
 * 
 * @author hnakagawa
...
public final class JSONConverter {
    ...
    private static ConcurrentMap<Class<?>, ImmutableHashMap<String, Field>> mBindCache = new ConcurrentHashMap<Class<?>, ImmutableHashMap<String, Field>>();

    private static ConcurrentMap<Class<?>, ImmutableHashMap<Field, String>> mPropCache = new ConcurrentHashMap<Class<?>, ImmutableHashMap<Field, String>>();

    /**
     * Class to convert JSON data to or from Java "primitive" types (including
     * Java primitive boxing types - e.g. Integer - and Strings). It basically
     * converts everything that doesn't translate into a JSONObject or JSONArray
     * or NULL.
     * 
     * This is not really necessary for types supported by the JSON library
     * (String, Integer, Double...) especially thanks to autoboxing, however
     * without this class other types like float wouldn't be supported as we
     * can't cast unrelated classes like the different numerical types.
     */
    private static abstract class PrimitiveConverter<T> {
        ...
        /**
         * Converts a T to an Object of a type which can be put() in a JSON
         * object or array.
         */
        protected Object convertToPutableObject(T value) {
            return value; // default, override for types non-supported by JSON
                          // classes
        }
    ...
    static {
        ...
        final PrimitiveConverter<String> jsonStringRetriever = new PrimitiveConverter<String>() {
            @Override
            public String retrieve(final JSONObject json, final String key) throws JSONException {
                return json.getString(key);
            }

            @Override
            public String retrieve(final JSONArray json, final int index) throws JSONException {
                return json.getString(index);
            }
        };
        final PrimitiveConverter<Boolean> jsonBooleanRetriever = new PrimitiveConverter<Boolean>() {
            @Override
            public Boolean retrieve(final JSONObject json, final String key) throws JSONException {
                return json.getBoolean(key);
            }

            @Override
            public Boolean retrieve(final JSONArray json, final int index) throws JSONException {
                return json.getBoolean(index);
            }
        };
        final PrimitiveConverter<Integer> jsonIntegerRetriever = new PrimitiveConverter<Integer>() {
            @Override
            public Integer retrieve(final JSONObject json, final String key) throws JSONException {
                return json.getInt(key);
            }

            @Override
            public Integer retrieve(final JSONArray json, final int index) throws JSONException {
                return json.getInt(index);
            }
        };
        final PrimitiveConverter<Long> jsonLongRetriever = new PrimitiveConverter<Long>() {
            @Override
            public Long retrieve(final JSONObject json, final String key) throws JSONException {
                return json.getLong(key);
            }

            @Override
            public Long retrieve(final JSONArray json, final int index) throws JSONException {
                return json.getLong(index);
            }
        };
        final PrimitiveConverter<Double> jsonDoubleRetriever = new PrimitiveConverter<Double>() {
            @Override
            public Double retrieve(final JSONObject json, final String key) throws JSONException {
                return json.getDouble(key);
            }

            @Override
            public Double retrieve(final JSONArray json, final int index) throws JSONException {
                return json.getDouble(index);
            }
        };
        final PrimitiveConverter<Float> jsonFloatRetriever = new PrimitiveConverter<Float>() {
            @Override
            public Float retrieve(final JSONObject json, final String key) throws JSONException {
                return ((Double) json.getDouble(key)).floatValue();
            }

            @Override
            public Float retrieve(final JSONArray json, final int index) throws JSONException {
                return ((Double) json.getDouble(index)).floatValue();
            }

            @Override
            protected Double convertToPutableObject(final Float value) {
                return value.doubleValue();
            }
        };
        final PrimitiveConverter<Short> jsonShortRetriever = new PrimitiveConverter<Short>() {
            @Override
            public Short retrieve(final JSONObject json, final String key) throws JSONException {
                return ((Integer) json.getInt(key)).shortValue();
            }

            @Override
            public Short retrieve(final JSONArray json, final int index) throws JSONException {
                return ((Integer) json.getInt(index)).shortValue();
            }

            @Override
            protected Integer convertToPutableObject(final Short value) {
                return value.intValue();
            }
        };
        final PrimitiveConverter<Byte> jsonByteRetriever = new PrimitiveConverter<Byte>() {
            @Override
            public Byte retrieve(final JSONObject json, final String key) throws JSONException {
                return ((Integer) json.getInt(key)).byteValue();
            }

            @Override
            public Byte retrieve(final JSONArray json, final int index) throws JSONException {
                return ((Integer) json.getInt(index)).byteValue();
            }

            @Override
            protected Integer convertToPutableObject(final Byte value) {
                return value.intValue();
            }
        };
        final PrimitiveConverter<Character> jsonCharRetriever = new PrimitiveConverter<Character>() {
            @Override
            public Character retrieve(final JSONObject json, final String key) throws JSONException {
                return (json.getString(key)).charAt(0);
            }

            @Override
            public Character retrieve(final JSONArray json, final int index) throws JSONException {
                return (json.getString(index)).charAt(0);
            }

            @Override
            protected String convertToPutableObject(final Character value) {
    ...
    private static Object retrieve(final Class<?> type, final JSONObject json, final String key,
            final Object enclosingObject) throws JSONConvertException {
        if (json.isNull(key))
            return null;
    ...
    @SuppressWarnings("unchecked")
    private static Object retrieve(final Class<?> type, final JSONArray json, final int index,
            final Object enclosingObject) throws JSONConvertException {
    ...
    private static void put(final JSONObject json, final String key, final Object value) throws JSONConvertException {
        if (value == null) {
            ...
            try {
                if (isPrimitivelyConvertible(type)) {
                    @SuppressWarnings("rawtypes")
                    // this is on purpose ; we need to call one of converter's
                    // generic-parameterized method, so as we don't know the
                    // type of 'value' at compile time, we need to explicitly
                    // call its bridge method
    ...
    // cf. comment below for unchecked //not currently used but keep as utility
    // method
    ...
    private static <T> T toObject(final JSONObject json, final Class<T> type, final Object enclosingInstance)
            throws JSONConvertException {
        try {
            final T obj;
            if (type.isMemberClass() && ((type.getModifiers() & Modifier.STATIC) != Modifier.STATIC)) { // if
                                                                                                        // non-static
                                                                                                        // member
                                                                                                        // (aka
                                                                                                        // inner)
    ...
    private static <T> T[] toObjectArray(final JSONArray json, Class<T> clazz, Object enclosingObject)
            throws JSONConvertException {
        @SuppressWarnings("unchecked")
    ...
    private static void bind(final JSONObject json, final Object obj) throws IllegalAccessException,
            InstantiationException, NoSuchMethodException, InvocationTargetException, JSONConvertException {
        ...
        for (String key : keySet) {
            ...
            if (value != null) // if null, we just let the Java object to its
                               // default value (or the one set in the
                               // constructor)
                FieldUtils.setNoException(obj, field, value);
        }
    }

    private static void bind(final JSONArray jsonArr, final Object[] objs, final Object enclosingObject)
            throws IllegalAccessException, InstantiationException, JSONConvertException {
    ...
    private static ImmutableHashMap<Field, String> createPropMap(Class<?> clazz) {
        ...
        for (Field field : fields) {
            if (!Modifier.isStatic(field.getModifiers()) && !field.isAnnotationPresent(Exclude.class)
                    && !field.isSynthetic()) { // ignore Exclude annotated
                                               // fields and synthetic fields
                field.setAccessible(true);
                String name = NamingConventionUtils.fromJavaFieldNameToJSONName(field.getName());
